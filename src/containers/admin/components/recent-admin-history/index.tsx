...
export default () => {
  const [isLoadingEditors, isErrorLoadingStaff, staffUsers] =
    useDatabaseQuery<UserAdminMeta>(
  ...
  if (Array.isArray(staffUsers) && staffUsers.length > staffMemberUserCap) {
    throw new Error(
      `Result should be less than the cap ${staffMemberUserCap} (got ${staffUsers.length})`
    )
  }
  ...
  const [isLoadingEntries, isErrorLoadingEntries, entries] =
    useDatabaseQuery<FullHistoryEntry>(
      'getfullhistory',
