...
export const loadUserIntoStore = async (userId, showLoading = true) => {
  if (!userId) {
    throw new Error('Cannot load user into store: no user ID provided!')
  }
  ...
  const user = await readRecord('getfullusers', userId)
